...
if (argv.from && argv.to) {
    ...
    launchChromeAndRunLighthouse(argv.url).then(results => {
        const prevReports = glob(`${dirPath}/*.json`, {
            sync: true
        });
        ...
        if (prevReports.length) {
            ...
            const recentReportDate = new Date(max).toISOString();
    
            const recentReportContents = getContents(dirPath + "/" + recentReportDate.replace(/:/g, "_") + ".json");
